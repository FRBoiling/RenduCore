        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_START:
                {
                    ...
                    {
                        Rendu::AllCreaturesOfEntryInRange pred(me, entry);
                        Rendu::CreatureListSearcher<Rendu::AllCreaturesOfEntryInRange> searcher(me, dummies, pred);
...
struct npc_infernal_rain_hellfire : public ScriptedAI
{
    ...
    void RebuildTargetList()
    {
        ...
        Rendu::AllCreaturesOfEntryInRange pred(me, NPC_INFERNAL_RAIN);
        Rendu::CreatureListSearcher<Rendu::AllCreaturesOfEntryInRange> searcher(me, others, pred);
        Cell::VisitAllObjects(me, searcher, 500.0f);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_CAST:
                {
                    if (Creature* target = ObjectAccessor::GetCreature(*me, Rendu::Containers::SelectRandomContainerElement(_targets)))
                    {
