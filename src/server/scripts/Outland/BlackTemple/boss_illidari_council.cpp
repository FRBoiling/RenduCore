...
static uint32 GetRandomBossExcept(uint32 exception)
{
    ...
    return Rendu::Containers::SelectRandomContainerElement(bossData);
}
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            case EVENT_BLESS:
            {
                ...
                Rendu::AnyFriendlyUnitInObjectRangeCheck checker(me, me, 100.0f);
                ...
                if (!TargetList.empty())
                {
                    Unit* target = Rendu::Containers::SelectRandomContainerElement(TargetList);
