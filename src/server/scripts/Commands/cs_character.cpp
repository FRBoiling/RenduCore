...
public:
    ...
    */
    ...
    static bool HandleCharacterChangeAccountCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string logString = Rendu::StringFormat("changed ownership of player %s (%s) from account %u to account %u", targetName.c_str(), targetGuid.ToString().c_str(), oldAccountId, newAccountId);
        if (WorldSession* session = handler->GetSession())
        {
            if (Player* player = session->GetPlayer())
