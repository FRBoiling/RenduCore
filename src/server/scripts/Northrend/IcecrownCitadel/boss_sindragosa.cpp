...
class spell_sindragosa_unchained_magic : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        for (WorldObject* target : targets)
        {
            ...
            if (Rendu::Helpers::Entity::IsPlayerHealer(player))
            {
            ...
            switch (player->GetClass())
            {
                ...
                case CLASS_WARLOCK:
                    ...
                    break;
                case CLASS_SHAMAN:
                    if (Rendu::Helpers::Entity::GetPlayerSpecialization(player) != SPEC_SHAMAN_ENHANCEMENT)
                        casters.push_back(target);
                    break;
                case CLASS_DRUID:
                    if (Rendu::Helpers::Entity::GetPlayerSpecialization(player) != SPEC_DRUID_FERAL)
                        casters.push_back(target);
                    break;
                default:
                    break;
        ...
        if (!healers.empty())
        {
            Rendu::Containers::RandomResize(healers, size_t(is25ManRaid ? 3 : 1));
            while (!healers.empty())
            {
        ...
        if (!casters.empty())
        {
            Rendu::Containers::RandomShuffle(casters);
...
class spell_frostwarden_handler_order_whelp : public SpellScriptLoader
{
    public:
        ...
        class spell_frostwarden_handler_order_whelp_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                targets.remove_if(Rendu::ObjectTypeIdCheck(TYPEID_PLAYER, false));
                if (targets.empty())
                    return;

                WorldObject* target = Rendu::Containers::SelectRandomContainerElement(targets);
                targets.clear();
            ...
            void HandleForcedCast(SpellEffIndex effIndex)
            {
                ...
                Rendu::Containers::SelectRandomContainerElement(unitList)->CastSpell(GetHitUnit(), uint32(GetEffectValue()), true);
            }
