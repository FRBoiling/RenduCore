...
class ValithriaDespawner : public BasicEvent
{
    public:
        ...
        {
            Rendu::CreatureWorker<ValithriaDespawner> worker(_creature, *this);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CHECK_PLAYER:
                        {
                            ...
                            Rendu::AnyPlayerInObjectRangeCheck check(me, 5.0f);
...
class spell_dreamwalker_summoner : public SpellScriptLoader
{
    public:
        ...
        class spell_dreamwalker_summoner_SpellScript : public SpellScript
        {
            ...
            {
                targets.remove_if(Rendu::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
                if (targets.empty())
                    return;
        ...
        class spell_dreamwalker_summon_suppresser_AuraScript : public AuraScript
        {
            ...
            void HandleSummon(Unit* caster)
            {
                ...
                for (uint8 i = 0; i < suppresserNumber; ++i)
                    caster->CastSpell(Rendu::Containers::SelectRandomContainerElement(summoners), SPELL_SUMMON_SUPPRESSER, true);
            }
