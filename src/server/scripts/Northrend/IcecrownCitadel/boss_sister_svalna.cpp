...
struct npc_crok_scourgebane : public EscortAI
{
    ...
    void WaypointStarted(uint32 waypointId, uint32 /*pathId*/) override
    {
        ...
        switch (waypointId)
        {
            ...
            case 4:
            {
                ...
                Rendu::CreatureListSearcher<FrostwingVrykulSearcher> searcher(me, temp, check);
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage) override
    {
        ...
        if (!_wipeCheckTimer)
        {
            ...
            Rendu::AnyPlayerInObjectRangeCheck check(me, 60.0f);
            Rendu::PlayerSearcher<Rendu::AnyPlayerInObjectRangeCheck> searcher(me, player, check);
            ...
            if (!player)
            {
                ...
                if (damage >= me->GetHealth())
                {
                    ...
                    Rendu::CreatureWorker<FrostwingGauntletRespawner> worker(me, respawner);
        ...
        while (uint32 eventId = Events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_ARNATH_PW_SHIELD:
                {
                    ...
                    DoCast(Rendu::Containers::SelectRandomContainerElement(targets), SPELL_POWER_WORD_SHIELD);
...
private:
    Creature* FindFriendlyCreature() const
    {
        ...
        Rendu::MostHPMissingInRange u_check(me, 60.0f, 0);
        Rendu::CreatureLastSearcher<Rendu::MostHPMissingInRange> searcher(me, target, u_check);
...
class spell_svalna_revive_champion : public SpellScript
{
    ...
    void RemoveAliveTarget(std::list<WorldObject*>& targets)
    {
        ...
        Rendu::Containers::RandomResize(targets, 2);
    }
