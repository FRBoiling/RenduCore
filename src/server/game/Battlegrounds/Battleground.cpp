...
void Battleground::SendBroadcastText(uint32 id, ChatMsg msgType, WorldObject const* target)
{
    ...
    Rendu::BroadcastTextBuilder builder(nullptr, msgType, id, GENDER_MALE, target);
...
uint32 Battleground::GetBonusHonorFromKill(uint32 kills) const
{
    ...
    return Rendu::Honor::hk_honor_at_level(maxLevel, float(kills));
}
...
void Battleground::SendMessageToAll(uint32 entry, ChatMsg msgType, Player const* source)
{
    ...
    Rendu::TrinityStringChatBuilder builder(nullptr, msgType, entry, source);
    Rendu::LocalizedPacketDo<Rendu::TrinityStringChatBuilder> localizer(builder);
    BroadcastWorker(localizer);
}
...
void Battleground::PSendMessageToAll(uint32 entry, ChatMsg msgType, Player const* source, ...)
{
    ...
    Rendu::TrinityStringChatBuilder builder(nullptr, msgType, entry, source, &ap);
    Rendu::LocalizedPacketDo<Rendu::TrinityStringChatBuilder> localizer(builder);
    BroadcastWorker(localizer);
