...
void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket* data, ObjectGuid guid, Player* player, BattlegroundTypeId bgTypeId, uint8 fromWhere)
{
    ...
    winner_kills = Rendu::Honor::hk_honor_at_level(player->GetLevel(), float(winner_kills));
    loser_kills = Rendu::Honor::hk_honor_at_level(player->GetLevel(), float(loser_kills));
...
BattlegroundTypeId BattlegroundMgr::GetRandomBG(BattlegroundTypeId bgTypeId)
{
    if (BattlegroundTemplate const* bgTemplate = GetBattlegroundTemplateByTypeId(bgTypeId))
    {
        ...
        return *Rendu::Containers::SelectRandomWeightedContainerElement(ids, weights);
    }
