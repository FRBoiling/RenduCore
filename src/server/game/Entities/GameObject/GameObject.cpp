...
void GameObject::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (m_spawnId)
            Rendu::Containers::MultimapErasePair(GetMap()->GetGameObjectBySpawnIdStore(), m_spawnId, this);
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (GetOwner())
                    {
                        ...
                        Rendu::NearestAttackableNoTotemUnitInObjectRangeCheck checker(this, radius);
                        Rendu::UnitLastSearcher<Rendu::NearestAttackableNoTotemUnitInObjectRangeCheck> searcher(this, target, checker);
                    ...
                    else
                    {
                        ...
                        Rendu::AnyPlayerInObjectRangeCheck checker(this, radius);
                        Rendu::PlayerSearcher<Rendu::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
...
/*static*/ bool GameObject::DeleteFromDB(ObjectGuid::LowType spawnId)
{
    ...
    sMapMgr->DoForAllMapsWithMapId(data->mapId,
        [spawnId, charTrans](Map* map) -> void
        {
            ...
            for (auto const& pair : Rendu::Containers::MapEqualRange(map->GetGameObjectBySpawnIdStore(), spawnId))
...
void GameObject::SaveRespawnTime(uint32 forceDelay)
{
    if (m_goData && (forceDelay || m_respawnTime > GameTime::GetGameTime()) && m_spawnedByDefault)
    {
        ...
        GetMap()->SaveRespawnTime(SPAWN_TYPE_GAMEOBJECT, m_spawnId, GetEntry(), thisRespawnTime, Rendu::ComputeGridCoord(GetPositionX(), GetPositionY()).GetId());
    }
}
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    Rendu::NearestGameObjectFishingHole u_check(*this, range);
    Rendu::GameObjectSearcher<Rendu::NearestGameObjectFishingHole> checker(this, ok, u_check);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
                        if (Player* target = ObjectAccessor::GetPlayer(*this, Rendu::Containers::SelectRandomContainerElement(m_unique_users)))
