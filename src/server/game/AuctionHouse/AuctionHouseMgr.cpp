...
std::string AuctionEntry::BuildAuctionMailSubject(MailAuctionAnswers response) const
{
    ...
    return Rendu::StringFormat("%u:%d:%u:%u:%u", itemEntry, item ? item->GetItemRandomPropertyId() : 0, response, Id, itemCount);
}

std::string AuctionEntry::BuildAuctionWonMailBody(ObjectGuid guid, uint32 bid, uint32 buyout)
{
    return Rendu::StringFormat("%llX:%u:%u", guid.GetRawValue(), bid, buyout);
}
...
{
    return Rendu::StringFormat("%llX:%u:%u:%u:%u", guid.GetRawValue(), bid, buyout, deposit, consignment);
}
...
{
    return Rendu::StringFormat("%llX:%u:%u:%u:%u:%u:%u", guid.GetRawValue(), bid, buyout, deposit, consignment, moneyDelay, eta);
}
