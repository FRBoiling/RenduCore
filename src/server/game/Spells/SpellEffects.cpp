...
void Spell::EffectAddHonor(SpellEffIndex /*effIndex*/)
{
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Rendu::Honor::hk_honor_at_level(unitTarget->GetLevel(), float(damage));
...
void Spell::EffectForceDeselect(SpellEffIndex /*effIndex*/)
{
    ...
    Rendu::MessageDistDelivererToHostile notifierBreak(unitCaster, &data, dist);
    Cell::VisitWorldObjects(unitCaster, notifierBreak, dist);

    // and selection
    data.Initialize(SMSG_CLEAR_TARGET, 8);
    data << uint64(unitCaster->GetGUID());
    Rendu::MessageDistDelivererToHostile notifierClear(unitCaster, &data, dist);
